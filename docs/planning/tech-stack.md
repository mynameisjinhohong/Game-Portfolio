...
챗봇은 단순 이력 조회 도구가 아니라 포트폴리오 전반을 안내하는 에이전트로 동작해야 한다.
시스템 프롬프트는 다음 세 영역을 모두 포함한다.

#### (A) 페르소나 및 기본 행동 규칙
- 챗봇의 이름, 말투, 응답 길이 가이드라인
- 예: "당신은 홍진호의 포트폴리오 안내 도우미입니다. 친근하되 전문적인 어조를 사용합니다."
- 욕설·부적절한 요청 시 정중히 거절하는 규칙 포함

#### (B) 이력·프로젝트 데이터
- 홍진호의 경력, 보유 기술, 프로젝트 목록 및 각 프로젝트의 역할·성과
- Itch.io 링크 등 외부 참조 URL
- 연락처 (이메일, GitHub)

#### (C) 이력과 무관한 질문에 대한 정형 응답 정의
다음 유형의 질문은 고정된 템플릿으로 응답하여 일관성을 유지한다.

| 질문 유형 | 정형 응답 방향 |
|-----------|---------------|
| 일반 코딩 질문 (포트폴리오 무관) | "저는 홍진호의 포트폴리오 안내를 위한 도우미입니다. 일반적인 프로그래밍 질문보다는 홍진호의 경험이나 프로젝트에 대해 물어봐 주세요." |
| 개인 정보·연락처 요청 | 이메일·GitHub 링크를 안내하고 직접 연락 유도 |
| 채용 제안·협업 요청 | 긍정적으로 수신하고 이메일로 연락 안내 |
| 챗봇 자체(AI 모델)에 관한 질문 | "저는 로컬에서 동작하는 AI 도우미입니다. 구체적인 모델 정보는 공개하지 않습니다." |
| 욕설·부적절한 내용 | 정중하게 거절, 주제 전환 유도 |

> 구현 파일: `lib/system-prompt.ts` — 세 영역을 함수로 조합해 최종 시스템 프롬프트 문자열 생성

### 2.4 초기 구현 범위

- [ ] 기본 대화 흐름 (질문 입력 → 스트리밍 응답 출력)
- [ ] 시스템 프롬프트 구성 (페르소나 + 이력 데이터 + 정형 응답 규칙)
...
| 항목 | 현재 결정 | 추후 검토 트리거 |
|------|-----------|-----------------|
| 챗봇 모델 | 로컬 Ollama | 응답 품질 불만족 시 다른 오픈소스 모델(Mistral, Phi-3 등)로 교체 검토 |
